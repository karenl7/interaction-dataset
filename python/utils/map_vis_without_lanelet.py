...
def draw_map_without_lanelet(filename, axes, lat_origin, lon_origin):
    ...
    for way in e.findall('way'):
        ...
        if way_type is None:
            ...
            type_dict = dict(color="black", linewidth=2, zorder=10, alpha=0.5)
        elif way_type == "line_thin":
            way_subtype = get_subtype(way)
            if way_subtype == "dashed":
                type_dict = dict(color="gainsboro", linewidth=1, zorder=10, dashes=[10, 10])
            else:
                type_dict = dict(color="gainsboro", linewidth=1, zorder=10)
        elif way_type == "line_thick":
            way_subtype = get_subtype(way)
            if way_subtype == "dashed":
                type_dict = dict(color="gainsboro", linewidth=2, zorder=10, dashes=[10, 10])
            else:
                type_dict = dict(color="gainsboro", linewidth=2, zorder=10)
        elif way_type == "pedestrian_marking":
            type_dict = dict(color="gainsboro", linewidth=1, zorder=10, dashes=[5, 10])
        elif way_type == "bike_marking":
            type_dict = dict(color="gainsboro", linewidth=1, zorder=10, dashes=[5, 10])
        elif way_type == "stop_line":
            type_dict = dict(color="gainsboro", linewidth=3, zorder=10)
